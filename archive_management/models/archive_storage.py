...
class ArchiveStorage(models.Model):
    ...
    repository_id = fields.Many2one(
        "archive.repository",
        readonly=True,
        related="repository_level_id.repository_id",
        store=True,
    )
    ...
    level = fields.Integer(
        related="repository_level_id.level", readonly=True, store=True
    )
    ...
    def default_archive_name(self, vals):
        level = self.env["archive.repository.level"].browse(
            vals.get("repository_level_id", False)
        )
        if level.sequence_id:
            return level.sequence_id._next()
        return self.env["ir.sequence"].next_by_code("archive.storage") or "/"
...
class ArchiveStorageParent(models.Model):
    ...
    _description = "archive.storage.parent"

    child_id = fields.Many2one("archive.storage", required=True, readonly=True)
    parent_id = fields.Many2one(
        "archive.storage", related="transfer_id.dest_storage_id", readonly=True
    )
    partner_id = fields.Many2one(
